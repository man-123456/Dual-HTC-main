...
class GraphLayer(nn.Module):
    ...
    def forward(self, label_emb, extra_attn, self_attn_mask, inputs_embeds, cross_attn_mask):
        if GRAPH == 'GRAPHORMER':
            ...
            graph_inputs = label_emb.expand(inputs_embeds.size(0), -1, -1)
...
class GraphEncoder(nn.Module):
    def __init__(self, config, tokenizer, graph=False, layer=1, data_path=None, threshold=0.01, tau=1):
    ...
    def forward(self, inputs_embeds, attention_mask, labels, embeddings):
        ...
        if self.graph:
            if GRAPH == 'GRAPHORMER':
                label_emb += self.id_embedding(self.label_id[:, None].expand(-1, expand_size)).view(1, -1, self.config.hidden_size)
                extra_attn = self.distance_embedding(self.distance_mat) + self.edge_embedding(self.edge_mat).sum(dim=1) / (self.distance_mat.view(-1, 1) + 1e-8)
        ...
        for hir_layer in self.hir_layers:
            label_emb, graph_inputs = hir_layer(label_emb, extra_attn, self_attn_mask, inputs_embeds, cross_attn_mask)

        token_probs = label_emb.mean(dim=1).view(attention_mask.size(0), attention_mask.size(1),self.label_name.size(0),)

        # sum
        contrast_mask = (F.gumbel_softmax(token_probs, hard=False, dim=-1, tau=self.tau) * labels.unsqueeze(1)).sum(-1)
        ...
        return contrast_mask, graph_inputs
