...
def get_data_from_meta():
    ...
    for line in origin_txt[1:]:
        ...
        doc = line[6]
        doc = clean_str(doc)
        # label1 = 'Civil Medical CS ECE MAE Psychology biochemistry'
        sample_text = doc
        # sample_text_label = sample_text
...
def split_train_dev_test():
    ...
    val = list(val)
    test = list(test)
    f = open('../wos/wos_train.json', 'w')
    f.writelines(train)
    f.close()
    f = open('../wos/wos_test.json', 'w')
    f.writelines(test)
    f.close()
    f = open('../wos/wos_val.json', 'w')
    f.writelines(val)
    f.close()
