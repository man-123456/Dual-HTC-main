...
class BertDataset(Dataset):
    def __init__(self, max_token=512, device='cpu:0', pad_idx=0, data_path=None):
        self.device = device
...
parser.add_argument('--name', type=str, default='train3-ZLPR-Graphormer-Lamb1=1-Labm2=0.5-lr=2e-5', help='A name for different runs.')
...
parser.add_argument('--lamb2', default=1, type=float, help='lambda')
parser.add_argument('--thre', default=0.05, type=float, help='Threshold for keeping tokens. Denote as gamma in the paper.')
...
parser.add_argument('--label_cpt', type=str, default='data/WebOfScience/WebOfScience.taxonomy', help='Label hierarchy file.')
...
if __name__ == '__main__':
    ...
    model = ContrastModel.from_pretrained('./PretrainedModel/bert-base-uncased',
                                          tokenizer=tokenizer,
                                          num_labels=num_class,
                                          contrast_loss=args.contrast, graph=args.graph,
                                          layer=args.layer, data_path=data_path, multi_label=args.multi,
                                          lamb=args.lamb, lamb2=args.lamb2, threshold=args.thre, tau=args.tau, label_cpt=args.label_cpt)
